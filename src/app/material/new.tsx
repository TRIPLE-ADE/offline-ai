...
export default function ImportMaterialScreen() {
  ...
  const choose = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'This file could not be imported.');
    } finally {
      setChoosing(false);
    }
  };
      ...
      <SafeAreaView style={styles.safeArea} edges={['bottom', 'left', 'right']}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <ScreenHeader
            ...
            title={draft ? 'Is this the right file?' : 'Import learning material'}
            subtitle={
              draft
          ...
          {!draft ? (
            ...
            <View style={[styles.fileCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <View style={[styles.fileIcon, { backgroundColor: theme.secondarySoft }]}>
                <Ionicons name="document-text-outline" color={theme.secondary} size={30} />
              </View>
              <View style={styles.flex}>
                <ThemedText type="heading" numberOfLines={3}>{draft.title}</ThemedText>
                <ThemedText themeColor="textSecondary">
                  {draft.fileType.toUpperCase()} · {formatSize(draft.fileSize)}
                </ThemedText>
              </View>
              <View style={[styles.privacyNote, { backgroundColor: theme.primarySoft }]}>
                <Ionicons name="lock-closed-outline" color={theme.primary} size={20} />
                <ThemedText type="small" style={styles.flex}>
                  A private copy is stored inside Soma. The original file is not changed.
                </ThemedText>
              </View>
            </View>
          )}
          ...
          {error ? (
            <View style={[styles.error, { backgroundColor: theme.dangerSoft }]}>
              <Ionicons name="alert-circle-outline" color={theme.danger} size={22} />
              <View style={styles.flex}>
                <ThemedText type="smallBold" style={{ color: theme.danger }}>Import needs attention</ThemedText>
        ...
        {draft ? (
          <View style={[styles.bottomActions, { backgroundColor: theme.background, borderTopColor: theme.divider }]}>
            <PrimaryButton
              label={saving ? 'Saving material…' : 'Import and continue'}
              leading={saving ? <ActivityIndicator color="#FFFFFF" /> : undefined}
              disabled={saving}
